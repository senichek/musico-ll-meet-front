...
const FilterMusicos = ({ setFinalUrl }) => {
    ...
    const handleFilterFormSubmit = (event) => {
        ...
        if (selectedGenre) {
            // here we will need only the Ids of the genres
            debugger
            usedInputs.push(selectedGenre);
            const url = `musical_type[]=${selectedGenre.id}`;
            urlParams.push(url);
        };
    ...
    const handleReset = () => {
        ...
        setSelectedCity("");
        setSelectedGenre("");
        // The change of finalUrl will trigger re-render of the list of Musicos
        setFinalUrl("");
    };
...
FilterMusicos.propTypes = {
    setFinalUrl: PropTypes.func
};

export default FilterMusicos;
